...
export const servey = ({ name, email, address, Phoneno, occupation, age,  education, hobbies, nationality,  adharno }) => (dispatch) => {
  // Headers
  const headers = {
    headers: {
      "Content-Type": "application/json"
  ...
  // Request body
  const body = JSON.stringify({ name, email, address, Phoneno, occupation, age,  education, hobbies, nationality,  adharno });

  axios
    .post("/api/users/profile", body, headers)
    .then((res) =>{
      dispatch(returnStatus(res.data, res.status, 'SURVEY_SUCCESS'));
      dispatch({ type: IS_LOADING })
    })
    .catch((err) => {
      dispatch(returnStatus(err.response.data, err.response.status, 'SURVEY_FAIL'))
      dispatch({
        type: SURVEY_FAIL
      });
      dispatch({ type: IS_LOADING })
